...
// User and Blog relationship (to enable populate)
...
//Deleting User's Blog upon Deleting User Profile
userSchema.pre<IUserModel>("remove", async function (next) {
  ...
  Logging.warn(
    `All Blogs created by ${user.name} has been deleted as the user deleted thier account`
  );
  next();
});
